...
FragTrap::~FragTrap(void)
{
	std::cout << this->_name << " rotted in the dustbin of the universe (FragTrap)"  << std::endl;
	return;
}
...
void	FragTrap::vaulthunter_dot_exe(std::string const & target)
{
	if (get_enp() >= get_vault_exe_cost())
	{
		...
		_enp -= _vault_exe_cost;
	}
	else
		std::cout << _name << " have no power of will to touch " << target << ", need " << \
		_vault_exe_cost - _enp << " energy point(s)" << std::endl;
}
