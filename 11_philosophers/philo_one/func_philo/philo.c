...
/*   Updated: 2021/03/30 23:40:00 by qmalcom          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../philo.h"

pthread_mutex_t g_messages_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void			philo_join(t_philo *table)
{
	...
	{
		if (pthread_join(table[i].life, NULL))
		{
			printf("%s", PTHREAD_JOIN_ERR);
			return ;
		}
		if (pthread_join(table[i].thread, NULL))
		{
			printf("%s", PTHREAD_JOIN_ERR);
			return ;
		}
...
void			*check_philo(void *args)
{
	...
	while (g_all_alive)
	{
		pthread_mutex_lock(philos->vitality);
		if (philo_check_meals_counter(philos) > 0 || !g_all_alive)
			break ;
		...
		pthread_mutex_unlock(philos->vitality);
		usleep(1000);
	}
	return (NULL);
}
