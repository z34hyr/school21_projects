...
Response::Response(Client *client, Request *req)
{
	...
	__session_enabled = client->getServer()->getSessionEnabled();
	__known_sessions = client->getServer()->getSessions();
	__current_cookie = client->getReqCookie();
	if (__current_cookie["total_count"].size())
		std::cout << "I see total counter\n";
	else
		std::cout << "Cant fiind key total counter\n";
	__mapped_headers = client->getRequest()->getMappedHeadrs();
	__user_id = client->getFd();
}
...
std::string	Response::joinCookie()
{
	std::string joined_cookie;
	std::map<std::string, std::string>::iterator i_b = __current_cookie.begin();
	std::map<std::string, std::string>::iterator i_e = __current_cookie.end();
	if (i_b == i_e)
		return "";
	while (i_b != i_e)
	{
		joined_cookie += "Set-Cookie: " + i_b->first + "=" + i_b->second + "\r\n";
		++i_b;
	}
	return joined_cookie;
}
...
int		Response::checkSessionID()
{
	std::cout << "	sess name " << __current_cookie["session_id"] << "\n";
	std::vector<std::string>::iterator it = std::find(__known_sessions.begin(), __known_sessions.end(), __current_cookie["session_id"]);
	if (it != __known_sessions.end() && *it != "")
		return 1;
	return 0;
}

std::string Response::getSessionID() { return __session_id; }

void	Response::prepareBody()
{
	__config = __req->config;
	std::ostringstream s2;
	if (__session_enabled)
	{
		if (!checkSessionID())
		{
			std::ostringstream s1;
			s1 << std::rand();
			__session_id = "user" + s1.str();
			__current_cookie["session_id"] = __session_id;
			__client->setSessionID(__session_id);
		}
		__client->addSessionCounter();
		s2 << __client->getSessionCounter();
		__current_cookie["session_counter"] = s2.str();
		// if (!__mapped_headers["Authorization"].size())
		// {
		// 	//401 - need auth
		// 	body = getAuthBody();
		// 	__status = 401;
...
std::string Response::getAuthBody()
{
	std::string	new_page = "";
	new_page += "<!DOCTYPE html>\n";
	new_page += "<html>\n";
	new_page += 	"\t<head>\n";
	new_page += 		"\t\t<title>Authorization needed</title>\n";
	new_page += 	"\t</head>\n";
	new_page += 	"\t<body style=\"background-color:#000000\">\n";
	new_page += "\t\t\t<h1 align=\"center\" style=\"color:#00ff15; font-size:350%\">401 Authorization needed</h1>\n";
	new_page += 	"\t</body>\n";
	new_page += "</html>\n";
	__file_type = "text/html";
	return new_page;
