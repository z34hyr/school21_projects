...
std::string	Response::joinCookie()
{
	...
	if (__req->getReqCookie()["session"].empty()) {
		if (!__client->getSessionID().size())
			__client->setSessionID(createSession());
		joined_cookie += "Set-Cookie: session=" + __client->getSessionID() + "\r\n";
	}
...
std::string Response::getHead()
{
	...
	s1 << __status;
	s2 << body.length();
	std::string first_string = __protocol + " " + s1.str() + " " + response_messages[__status] + "\r\n";
	first_string += "Date: " + getDate() + "\r\n";
	first_string += "Server: " + __server_name + "\r\n";
	first_string += "Content-length: " + s2.str() + "\r\n";
	if (__file_type.size())
...
char **Response::getCgiEnv()
{
	...
	std::map<std::string, std::string> temp = __req->getReqCookie();
	...
	if (temp.empty())
		return env;
	std::map<std::string, std::string>::iterator i_b = temp.begin();
	std::map<std::string, std::string>::iterator i_e = temp.end();

	for (; i_b != i_e; i_b++) {
		http_cookie += i_b->first;
		http_cookie += "=";
		http_cookie += i_b->second;
		http_cookie += ";";
	}
	env[0] = strdup(http_cookie.c_str());
	return env;
}
/*
** --------------------------------- ACCESSOR ---------------------------------
